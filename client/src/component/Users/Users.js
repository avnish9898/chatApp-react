...
function Users({ usersOnline, image, name, id, socketId, urlPath }) {
  ...
  const search = () => {
    ...
    let obj = allUsers.filter((ob) => {
      ...
      if (
        ob._id != id &&
        (ob.name.toLowerCase().includes(searchValue) ||
          ob.username.toLowerCase().includes(searchValue))
      ) {
        return ob;
      }
    });
  ...
  return (
    <div className={urlPath ? "userList fullScreen" : "userList"}>
      <div className="userList__heading">
        <img
          src={image ? "http://localhost:4500/profile/dp/" + image : ""}
          className="userList__heading__left"
        />

        <div className="userList__heading__right">{name}</div>
        <Dropdown />
      </div>
      ...
      {searchUsers != null ? (
        ...
        <>
          <div className="userList__tabs">
            <button
              className={` ${toggleClass == 0 && "tabHighlight"}`}
              onClick={() => tabClick(0)}
            >
              chats
            </button>
            <button
              className={` ${toggleClass == 1 && "tabHighlight"}`}
              onClick={() => tabClick(1)}
            >
              all
            </button>
            <button
              className={` ${toggleClass == 2 && "tabHighlight"}`}
              onClick={() => tabClick(2)}
            >
              online
            </button>
          ...
          {toggleClass == 0 ? (
            ...
            <UsersList userArray={usersOnline} />
          )}
        </>
      )}
    </div>
